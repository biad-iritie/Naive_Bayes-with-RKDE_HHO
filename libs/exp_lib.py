...
class Density_model:
    ...
    def compute_score(self, true_dens):
        ...
        self.kullback_f0_f = metrics.kl(true_dens.reshape(
            (-1, 1)), self.density.reshape((-1, 1)))
        self.kullback_f_f0 = metrics.kl(
            self.density.reshape((-1, 1)), true_dens.reshape((-1, 1)))
        self.jensen = metrics.js(self.density.reshape(
            (-1, 1)), true_dens.reshape((-1, 1)))
        ...
        if self.algo == 'kde':
            ...
            for i_d in range(d):
                ...
                K_plot = K_plot + (np.dot(np.ones((m, 1)), temp_x.T) -
                                   np.dot(temp_xpos, np.ones((1, n_samples))))**2
