...
def load_data(dataset):
    ...
    print("Loaded {} data: {} samples, {} dimensions, {} labels".format(
        dataset, X.shape[0], X.shape[1], len(set(y))))
...
def generate(param_gauss,
    ...
    X1 = np.random.normal(
        param_gauss[0], param_gauss[2], size=(n_inliers // 2, dim))
    X2 = np.random.normal(
        param_gauss[1], param_gauss[3], size=(n_inliers // 2, dim))
    X = np.r_[X1, X2]
    n_inliers = X.shape[0]
...
def true_density(gauss_param, X_plot):
    ...
    if d > 1:
        true_dens = coeff * sci_multinorm((gauss_param[0], gauss_param[0]), gauss_param[2]).pdf(
            X_plot) + coeff * sci_multinorm((gauss_param[1], gauss_param[1]), gauss_param[3]).pdf(X_plot)
    return true_dens
